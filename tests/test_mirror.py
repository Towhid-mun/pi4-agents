...
The transfer itself is proved by P0-3's done-test against the real target.
"""
...
class TestRsyncArgv(unittest.TestCase):
    def test_shape(self):
        argv = mirror.rsync_argv(make_config(), "/tmp/ex.txt")
    ...
    def test_absolute_remote_root(self):
        argv = mirror.rsync_argv(make_config(remote_root="/srv/blinky"), "/tmp/ex.txt")
        self.assertEqual(argv[-1], "pi:/srv/blinky/")


class TestMkdirArgv(unittest.TestCase):
    def test_creates_the_remote_root(self):
        argv = mirror.mkdir_argv(make_config())
        self.assertEqual(argv, ["ssh", "pi", "mkdir -p projects/blinky"])

    def test_remote_path_is_shell_quoted(self):
        argv = mirror.mkdir_argv(make_config(remote_root="proj/my stuff"))
        self.assertEqual(argv[2], "mkdir -p 'proj/my stuff'")

    def test_a_hostile_remote_root_cannot_break_out_of_the_argument(self):
        argv = mirror.mkdir_argv(make_config(remote_root="proj; rm -rf ~"))
        self.assertEqual(argv[2], "mkdir -p 'proj; rm -rf ~'")
